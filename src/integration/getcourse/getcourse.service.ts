...
const { GC_DOMAIN, GC_SECRET_KEY, BH_CLIENT_ID, BH_CLIENT_SECRET } = process.env;
...
export class GetcourseService {
    ...
    async getcourseNewOrder(query) {
        ...
        const contact = await this.contactsService.getOneContactByEmail({
            where: { contact_email },
        });
        if (!contact) {
    ...
    async updateGetcourseUserByEmail(email: string, chatId: string) {
        console.log('Запустилась функция обновления поля пользователя');
        const data = {
            user: {
                email: email,
                addfields: {
        ...
        console.log('Закончилась функция обновления поля пользователя', result);
    }
