...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Функция для поиска input по тексту лейбла
    function findInputByLabelText(labelText) {
        // Находим все лейблы на странице
        const labels = document.querySelectorAll('label.field-label');

        // Перебираем лейблы
        for (const label of labels) {
            // Проверяем, содержит ли лейбл нужный текст
    ...
    // Если utm_source есть, ищем поле и вставляем значение
    if (utmSource && utmSource.trim() !== '') {
        const inputField = findInputByLabelText('Источник');
        if (inputField) {
            inputField.value = utmSource; // Вставляем значение utm_source в поле
        }
    }
});
